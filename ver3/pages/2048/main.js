...
Main.prototype = {
  ...
  getvalue(nomal){
    if(nomal){
      ...
      else if (0.4 < Math.random() <= 0.7){
        var value = 4
      }
      else if (0.7 < Math.random() <= 0.8){
        var value = 8
      }
      else if (0.8 < Math.random() <= 0.9){
        var value = 16
      }
      else if (0.9 < Math.random() <= 1){
        var value = 32
      }
    }else{
  ...
  addRandomData() { //填充数据
    if(!this.board.cellEmpty()) {
      ...
      const nomal = wx.getStorageSync('nomal');
      var value = this.getvalue(nomal);
      cell.val = value; 
      this.update(cell);
    }
  },
